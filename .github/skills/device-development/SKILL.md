...
- MIoT event schema 只声明事件的物理定位和逻辑 alias；argument 顺序和属性 metadata 来自完整物理 spec，不要在 schema 中重复声明。共享 MIoT 层将 raw argument 保留为 `unknown`，只验证数量、PIID 配对、唯一性和地址等路由所必需的结构；值类型与语义由 concrete device connection 解码。
- 若领域事件依赖事件后的关联状态，在 provider 内完成事件后的读取和状态提交，再发布 `DeviceEvent`；读取失败时从当前 available observation 产生 `undefined`，不要沿用 last-known 旧值冒充本次观测。可以合并密集事件的关联状态读取，但只能承诺 callback 前已有该批次的最新 available observation，不能声称每个 occurrence 都拥有独立的物理采样。如果领域语义要求一一对应，应串行读取或把设备真正与 event 一起上报的 sample 放进具体 event class。优先使用能满足时序的本地链路，只有实机证据表明某个属性来源不可靠时才增加属性级来源策略。
